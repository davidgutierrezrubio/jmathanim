...
You need to add the dependencies to add the JMathAnim library to your project. You can use the latest released version (right now v0.9.8) or a snapshot version. Currently, I recommend using development snapshots because released versions <1.0.0 may still have bugs, that are periodically fixed in the snapshot versions before releasing a new one.
