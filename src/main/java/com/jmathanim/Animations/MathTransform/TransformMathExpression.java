...
//        scene.remove(mshDst);
	 ...
	 * Current transform will be done via the Transform method, interpolating point
	 * by point. It will return an associated TransformMathExpressionParameters
	 * instance so that effects can be added to the transform.
	 *
	...
	/**
	 * Overloaded method. Maps the origin group defined with the given name to the
	 * jth-element of destiny formula.
	 *
	 ...
	 * TransformMathExpressionParametersArray to add uniformly the same effects to
	 * all maps created
	 *
	...
	/**
	 * Map all index from origin to destiny. For this to work, destiny size has to
	 * be greater or equal than origin size.
	 *
	 * @return Associated array transform parameter, to add effects.
	 */
	...
	/**
	 * Sets the removing style for the specified origin indices marked for removal
	 *
	 * @param type    One of the enum RemoveType: FADE_OUT, SHRINK_OUT, MOVE_OUT_UP,
	 *                MOVE_OUT_LEFT, MOVE_OUT_RIGHT, MOVE_OUT_DOWN
