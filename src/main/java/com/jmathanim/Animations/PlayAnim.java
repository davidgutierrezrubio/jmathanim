...
/**
 * Several static methods to easily perform most common animations. This class is automatically instantiated by the
 * JMathAnimScene class for using in the scene.
 *
...
public class PlayAnim {
    ...
    /**
     * Generates a new PlayAnim object. This class is used internally by JMathAnim to hold fast access to most common
     * animations
     *
    ...
    /**
     * Animates the given objects, setting their alpha (draw and fill) from 0 to current, using default runtime. Then
     * add the objects to the scene if they aren't.
     *
    ...
    /**
     * Animates the given objects, setting their alpha (draw and fill) from 0 to current, using given runtime.Then add
     * the objects to the scene if they aren't.
     *
    ...
    /**
     * Animates the given objects, setting their alpha (draw and fill) from current to 0, using default runtime. Then
     * remove the objects to the scene.
     *
     * @param objs Mathobjects to animate (varargs)
     */
    ...
    /**
     * Animates the given objects, setting their alpha (draw and fill) from current to 0, using given runtime.Then
     * remove the objects to the scene.
     *
     * @param runtime Duration in seconds
     * @param objs    Mathobjects to animate (varargs)
    ...
    /**
     * Animates all the objects in the scene, setting their alpha (draw and fill) from current to 0, using specified
     * runtime.Then remove the objects to the scene. This method is mostly used as a transition between parts of a
    ...
    /**
     * Shift the specified objects out of the math view and removes them from the scene.
     *
     * @param runtime     Duration in seconds
    ...
    /**
     * Animates a scaling of the given objects uniformly with a given scale, around a specified center
     *
     * @param runTime     Duration in seconds
    ...
    /**
     * Animates a scaling of the given objects uniformly with a given scale. The scale center is the center of the
     * combined bounding box of all objects. If you want to scale each object around its own center, use one scale
    ...
    /**
     * Animates a scaling of the given objects with a x and y scales.The scale center is the center of the combined
     * bounding box of all objects.If you want to scale each object around its own center, use one scale animation for
     * each object.
     *
     * @param runTime Duration in seconds
    ...
    /**
     * Animates a scaling of the given objects with a x, y and z scales.The scale center is the center of the combined
     * bounding box of all objects.If you want to scale each object around its own center, use one scale animation for
     * each object.
     *
     * @param runTime Duration in seconds
    ...
    /**
     * Animates a scaling of the given objects with given x and y scale, around a specified center
     *
     * @param runTime Duration in seconds
     * @param center  Scale center
     * @param scx     x-scale
    ...
    /**
     * Animates a scaling of the given objects with given x, y and z scale, around a specified center.This method is
     * defined for convenience as the z-scale has no effect until 3D is developed.
     *
     * @param runTime Duration in seconds
     * @param center  Scale center
     * @param scx     x-scale
    ...
    /**
     * Animates a rotation of the given objects around the center of the combined bounding box of all objects. If you
     * want to rotate each object around its own center, use one rotate animation for each object.
     *
     * @param runTime Duration in seconds
    ...
    /**
     * Animates a smooth transform from one MathObject to another. The specific type of transform is chosen depending on
     * the given objects. Currently there are specific implementations FunctionSimpleInterpolateTransform,
     * IsomorphicTransformAnimation, PointInterpolationCanonical, PointInterpolationSimpleShapeTransform, and
     * RotateAndScaleXYStrategyTransform. An optimization strategy is also chosen, if available. In general, the
     * transformed object may be unusable after the transform, so it's recommended to remove the transformed object from
     * the scene and using the destiny object instead.
     *
     * @param runTime     Duration in seconds
...
//    }
    ...
    /**
     * Zooms the default camera so that it contains the given rect. The resulting mathview is the smallest view
     * containing the Rect specified.
     *
     * @param runTime Duration in seconds
    ...
    /**
     * Animate a zoom over the current math view. The rect that represents the math view is multiplied by scale factor,
     * so factors greater than 1 means zoom out.
     *
     * @param scale   Scale factor. A scale of value .5 means applying a x2 zoom factor
     * @param runTime Duration in seconds
     */
...
//        scene.playAnimation(Commands.cameraZoomToRect(runTime, cam, cam.getMathView().scale(scale, scale)));
    ...
    /**
     * Overloaded method. Animates a camera pan with the given shift vector, specified by x and y coordinates
     *
     * @param runTime Duration in seconds
    ...
    /**
     * Plays an animation highlighting an object. Scales and unscales this object for 1 second. Objects not scalable
     * ({@link Point} for example) are not affected.
     *
    ...
    /**
     * Plays an animation highlighting an object.Scales and unscales this object for given time. Objects not scalable
     * ({@link Point} for example) are not affected.
     *
     * @param runTime Duration in seconds
     * @param mobjs   Objects to highlight (varargs)
    ...
    /**
     * Plays an animation to introduce an object into scene.Scaling, rotating and applying alpha from 0 to current.
     *
    ...
    /**
     * Plays an animation to introduce an object into scene.Scaling and applying alpha from 0 to current.
     *
     * @param mobjs   Objects to animate (varargs)
     * @param runTime Duration in seconds
     */
    ...
    /**
     * Convenience method.Plays an animation to introduce an object into scene. Scaling and applying alpha from 0 to
     * current. Duration of the animation is 2 seconds.
     *
     * @param mobjs Objects to animate (varargs)
     */
    ...
    /**
     * Plays an animation that reduces the size and alpha of the MathObject.A rotation of a given angle is performed
    ...
    /**
     * Plays an animation that reduces the size and alpha of the MathObject.After finishing the animation, object is
    ...
    /**
     * Convenience method. Plays an animation that reduces the size and alpha of the MathObject, with a duration of 1
     * second.After finishing the animation, object is removed from the current scene.
     *
    ...
    /**
     * Convenience overloaded method. Plays an animation drawing a MathObject with runtime of 2 seconds.
     *
     * @param mobjs Objects to highlight (varargs)
     */
    ...
    /**
     * Changes the draw and fill color of the selected objects, animating the change for the specified time. If any of
     * the colors is null, this color will not be animated. This way you can for example, change only the draw color of
     * the selected objects if fill color is null
     *
    ...
    /**
     * Changes the draw parameters of the selected objects to match the given style, animating the change for the
     * specified time.
     *
     * @param runtime   Time of animation (in seconds)
...
//    }
    ...
    /**
     * Plays an animation that hightlights the contour of a Shape object, with the specified duration
     *
     * @param runTime time in seconds
     * @param objs    MathObjects to animate (varargs)
     */
