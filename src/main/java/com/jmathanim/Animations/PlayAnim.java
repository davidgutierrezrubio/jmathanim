...
public class PlayAnim {
	...
	/**
	 * Animates the given objects, setting their alpha (draw and fill) from current
	 * to 0, using default runtime. Then remove the objects to the scene.
	 *
	 * @param objs Mathobjects to animate (varargs)
	 */
	...
	/**
	 * Animates the given objects, setting their alpha (draw and fill) from current
	 * to 0, using given runtime.Then remove the objects to the scene.
	 *
	 * @param runtime Duration in seconds
	 * @param objs    Mathobjects to animate (varargs)
	 */
	...
	/**
	 * Animates all the objects in the scene, setting their alpha (draw and fill)
	 * from current to 0, using default runtime {@link defaultRunTimefadeOut}. Then
	 * remove the objects to the scene. This method is mostly used as a transition
	...
	/**
	 * Animates all the objects in the scene, setting their alpha (draw and fill)
	 * from current to 0, using specified runtime.Then remove the objects to the
	 * scene. This method is mostly used as a transition between parts of a scene,
	 * as it clears the scene completely.
	 *
	 * @param runtime Duration in seconds
	 */
	...
	/**
	 * Shift the specified objects out of the math view and removes them from the
	 * scene.
	 *
	 * @param runtime     Duration in seconds
	 * @param exitAnchor  Exit, given by a Anchor constant. For example Anchor.UPPER
	 *                    will move the objects to the upper side of the math view.
	...
	/**
	 * Shift the specified objects out of the math view and removes them from the
	 * scene.The duration of the animation is the value stored in
	 ...
	 *                    Anchor.UPPER will move the objects from the upper side of
	 *                    the math view.
	 * @param mathObjects Mathobjects to animate (varargs)
	 */
	...
	/**
	 * Shift the specified objects from an outer point of the math view to their
	 * original position, adding them to the scene. The duration of the animation is
	 * the value stored in {@link defaultRunTimeMoveIn}.
	 *
	 * @param enterAnchor Enter point, given by a Anchor constant. For example
	 *                    Anchor.UPPER will move the objects from the upper side of
	 *                    the math view.
	...
	/**
	 * Animates a scaling of the given objects uniformly with a given scale, around
	 * a specified center
	 *
	 * @param runTime     Duration in seconds
	 ...
	 * scale center is the center of the combined bounding box of all objects. If
	 * you want to scale each object around its own center, use one scale animation
	 * for each object.
	 *
	 * @param runTime     Duration in seconds
	 * @param scaleFactor Scale factor
	 ...
	 * center is the center of the combined bounding box of all objects.If you want
	 * to scale each object around its own center, use one scale animation for each
	 * object.
	 *
	 * @param runTime Duration in seconds
	...
	/**
	 * Animates a scaling of the given objects with given x and y scale, around a
	 * specified center
	 *
	 * @param runTime Duration in seconds
	 * @param center  Scale center
	...
	/**
	 * Animates a scaling of the given objects with given x, y and z scale, around a
	 * specified center.This method is defined for convenience as the z-scale has no
	 * effect until 3D is developed.
	 *
	 * @param runTime Duration in seconds
	 * @param center  Scale center
	 * @param scx     x-scale
	...
	/**
	 * Animates a rotation of the given objects around the center of the combined
	 * bounding box of all objects. If you want to rotate each object around its own
	 * center, use one rotate animation for each object.
	 *
	 * @param runTime Duration in seconds
	...
	/**
	 * Animates a smooth transform from one MathObject to another. The specific type
	 * of transform is chosen depending on the given objects. Currently there are
	 * specific implementations FunctionSimpleInterpolateTransform,
	 * HomothecyStrategyTransform, PointInterpolationCanonical,
	 * PointInterpolationSimpleShapeTransform, and
	 * RotateAndScaleXYStrategyTransform. An optimization strategy is also chosen,
	 * if available. In general, the transformed object may be unusable after the
	 * transform, so it's recommended to remove the transformed object from the
	 * scene and using the destiny object instead.
	 *
	 * @param runTime     Duration in seconds
	 ...
	 * camera animations. You can set the gaps between objects and mathview borders,
	...
	/**
	 * Performs a pan and zoom out animation of the current camera to ensure all
	 * objects in the scene fit in the math view, using the specified run time.You
	 * can set the gaps between objects and mathview borders, with the
	 ...
	 * animations.You can set the gaps between objects and mathview borders, with
	...
	/**
	 * Performs a pan and zoom out animation of the current camera to ensure all
	 * given objects fit in the math view, using the specified run time. You can set
	 * the gaps between objects and mathview borders, with the camera.setGaps
	...
	/**
	 * This method is similar to adjustToObjects, but it performs a zoom in to use
	 * all availabla math view to show the specified objects
	 *
	 * @param runTime Duration in seconds
	 * @param objs    Mathobjects to include in the view (varargs)
	 */
	...
	/**
	 * Zooms the camera so that it contains the given rect. The resulting mathview
	 * is the smallest view containing the Rect specified.
	 *
	 * @param runTime Duration in seconds
	...
	/**
	 * Animate a zoom over the current math view. The rect that represents the math
	 * view is multiplied by scale factor, so factors greater than 1 means zoom out.
	 *
	 ...
	 * for 1 second. Objects not scalable ({@link Point} for example) are not
	 * affected.
	 *
	...
	/**
	 * Plays an animation highlighting an object.Scales and unscales this object for
	 * given time. Objects not scalable ({@link Point} for example) are not
	 * affected.
	 *
	 * @param runTime Duration in seconds
	 * @param mobjs   Objects to highlight (varargs)
	...
	/**
	 * Plays an animation to introduce an object into scene.Scaling, rotating and
	 ...
	 * Scaling and applying alpha from 0 to current. Duration of the animation is 2
	 * seconds.
	 *
	 * @param mobjs Objects to animate (varargs)
	 */
	...
	/**
	 * Plays an animation that reduces the size and alpha of the MathObject.After
	...
	/**
	 * Convenience method. Plays an animation that reduces the size and alpha of the
	 * MathObject, with a duration of 1 second.After finishing the animation, object
	 * is removed from the current scene.
	 *
	 ...
	 * chosen: For a simple shape, draws the shape. For a {@link MultiShapeObject}
	 * performs a simple shape creation for each shape, with a time gap between one
	 * and the next. For a {@link SVGMathObject} (which includes
	 * {@link LaTeXMathObject}) a "first draw, then fill" strategy is chosen.
	 *
	...
	/**
	 * Convenience overloaded method. Plays an animation drawing a MathObject with
	...
	/**
	 * Changes the draw and fill color of the selected objects, animating the change
	 * for the specified time. If any of the colors is null, this color will not be
	 * animated. This way you can for example, change only the draw color of the
	 * selected objects if fill color is null
	 *
	...
	/**
	 * Changes the draw parameters of the selected objects to match the given style,
	 * animating the change for the specified time.
	 *
	 * @param runtime   Time of animation (in seconds)
	 ...
	 * several objects are animated, the second will be stacked to the first, and so
	 * on
	 *
