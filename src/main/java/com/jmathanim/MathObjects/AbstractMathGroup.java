...
public abstract class AbstractMathGroup<T extends AbstractMathGroup<T>>
    ...
    abstract protected T makeNewInstance();

    private T flatten(AbstractMathGroup<?> group) {
        T resul = makeNewInstance();
        for (MathObject<?> obj : group) {
            if (obj instanceof AbstractMathGroup<?>) {
                AbstractMathGroup<?> cc = flatten(((AbstractMathGroup<?>) obj));
     ...
     * @return This MathObjectGroup
     */
    public T addWithKey(String key, MathObject<?> obj) {
        add(obj);
        dict.put(key, obj);
        return (T) this;
    }
     ...
     */
    public T distribute(MathObjectGroup.Orientation orientation) {
     ...
     * @return This object
     */
    public T distribute(MathObjectGroup.Orientation orientation, boolean respectOrder) {
        ArrayList<MathObject<?>> objectsToDistribute = new ArrayList<>(getObjects());
        if (respectOrder) {
            if (orientation == MathObjectGroup.Orientation.HORIZONTAL) {
...
//and sum of widths (heights)
    ...
    public T setLayout(GroupLayout layout) {
...
//            objects.get(n).stackTo(objects.get(n - 1), anchor, gap);
     ...
     * @return This group
     */
    public T homogeneizeBoundingBoxes(InnerAnchorType anchorType, double upperGap, double rightGap, double lowerGap, double leftGap) {
        double hmax = 0;
        double wmax = 0;
     ...
     * @return This object
     */
    public T homogeneizeBoundingBoxesTo(InnerAnchorType anchorType, double width, double height, double upperGap, double rightGap, double lowerGap, double leftGap) {
