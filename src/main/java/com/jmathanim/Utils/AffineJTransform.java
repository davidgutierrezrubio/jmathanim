...
public class AffineJTransform {
	...
	/**
	 * Matrix that stores the transform, with the following form: {{1, x, y, z}, {0,
	 * vx, vy, vz},{0, wx, wy, wz},{0 tx ty tz}} Where x,y,z is the image of (0,0,0)
	 * and v,w,z are the images of canonical vectors.
	 */
	 ...
	 *
	 * @param rm Matrix with the following form: {{1, x, y, z}, {0, vx, vy, vz},{0,
	 *           wx, wy, wz},{0 tx ty tz}} Where x,y,z is the image of (0,0,0) and
	...
	/**
	 * Sets the imagen of the origin by this transform In practice, it changes the
	 * first row of the matrix transform
	 *
	...
	/**
	 * Sets the imagen of the first canonical vector (1,0,0) by this transform In
	 * practice, it changes the second row of the matrix transform
	 *
	...
	/**
	 * Sets the imagen of the second canonical vector (0,1,0) by this transform In
	 * practice, it changes the third row of the matrix transform
	 *
	...
	/**
	 * Sets the imagen of the third canonical vector (0,0,1) by this transform In
	 * practice, it changes the fourth row of the matrix transform
	 *
	...
	/**
	 * Apply the current transform to the given MathObject. Strategy transformation
	 * depends on the type of MathObject. The object is modified.
	 *
	 * For MultiShapeObject objects iterates the transformation over all its shapes
	 *
...
//        JMathAnimScene.logger.warn("Don't know how to perform an Affine Transform on object " + mObject.getClass().getName());
	...
	/**
	 * Returns a copy of the transformed object. Original object is not modified.
	 *
	 * @param <T> Object type
	 ...
	 * C=A.compose(B) The resulting transform C,applied to a point, will result in
	 * applying first A and then B. Mathematically C(x)=B(A(x))
	 *
	...
	/**
	 * Creates a transform which scales from a given point by a factor uniformly.
	 *
	...
	/**
	 * Overloaded method. Creates a transform,which scales from a given point by a x
	 * and y factor uniformly. The z-factor is 1.
	 *
	 * @param center Scale center
	...
	/**
	 * Overloaded method. Creates a transform,which scales from a given point by a
	 * x, y and z factor uniformly.
	 *
	 * @param center Scale center
	 * @param scalex x-scale factor
	 * @param scaley y-scale factor
	...
	/**
	 * Creates a 2D homothecy transform in the plane (a rotation+traslation+uniform
	 * scale) which transforms the point A into point C and point B into point D.
	 * There are 2 such transforms that accomplish this, inverse and direct. This
	 * method returns the direct. To obtain the inverse method, a composition with a
	 * reflection of axis B-D should be done.
	 *
	 ...
	 * @param alpha Alpha parameter to animate the transform. 0 means unaltered. 1
	 *              means the full transform done.
	 * @return The transform
	 */
...
//        AffineTransform resul = new AffineTransform();
	...
	/**
	 * Overloaded method. Returns the affine transform that maps the rectangle r1
	 * onto r2
	 *
