...
//    }
	...
	/**
	 * Computes coordinates of the intersection of this Rect with the line defined
	 * by the coordinates x1,y1,x2,y2 (2D version)
	 *
	 *
	 ...
	 * @return A 4-tuple of coordinates, representing the 2 intersection points of
	 *         the line with the rect.
	 */
...
//    }
	/**
	 * Returns the smallest {@link Rect} which contains 2 given rects. The method is
	 * made static so that it can deal with the case any of them is null
	 *
	...
	public static Rect union(Rect a, Rect b) {
		...
		return new Rect(Math.min(a.xmin, b.xmin), Math.min(a.ymin, b.ymin), Math.max(a.xmax, b.xmax),
	...
	public Rect interpolate(Rect rDst, double t) {
		return new Rect((1 - t) * xmin + t * rDst.xmin, (1 - t) * ymin + t * rDst.ymin, (1 - t) * xmax + t * rDst.xmax,
	...
	/**
	 * Computes a new Rect which is the original grown by a horizontal and vertical
	 * gap. The original Rect is unaffected.
	 *
	...
	/**
	 * For compatilibiy purposes from previous versions. It has identical effects
	 * than scale method
	 *
	 * @param xs x scale
	 * @param ys y scale
	...
	/**
	 * Scale the rectangle around center, and return a new one with the result. Does
	 * not affect the current rect.
	 *
	 * @param xs x scale
	 * @param ys y scale
	 * @return The scaled rectangle.
	...
	/**
	 * Gets the point inside the rect with the relative coordinates from 0 to 1. The
	 * point (0,0) refers to the DL corner, the (1,1) the UR corner, and (.5,.5) the
	 * center of the rect
	 *
