...
public class BoxLayout extends AbstractBoxLayout {
	 ...
	 *                  depends on the direction chosen. For example if direction is
	 *                  RIGHT_UP, the corner will be the lower left one. If
	 *                  direction is DOWN_LEFT, the corner will be the upper right
	 *                  one.
	 * @param rowSize   Numbers of element in each row. Note that "row" becomes
	 *                  "columns" if direction is UP_LEFT, UP_RIGHT, DOWN_LEFT or
	 *                  DOWN_RIGHT
	 * @param direction Direction of the box. Specifies the direction to stack the
	 *                  elements. A direction of RIGHT_UP will stack the row in the
	 *                  RIGHT direction and then UP to allocate the next row
	 ...
	 * each row. So for example, getRowGroups(group).get(0) will return a
	...
	/**
	 * Returns a MathObjectGroup containing other MathObjectGroup objects, one for
	 * each column. So for example, getColGroups(group).get(0) will return a
