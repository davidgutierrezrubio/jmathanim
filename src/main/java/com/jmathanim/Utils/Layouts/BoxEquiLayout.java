 ...
 * strategy, with equally space. The space is computed using the max width and
 * height of elements in the group.
 *
...
public class BoxEquiLayout extends AbstractBoxLayout {
    ...
    public BoxEquiLayout(int rowSize) {
     ...
     */
    public BoxEquiLayout(Point corner, int rowSize) {
     ...
     */
    public BoxEquiLayout(Point corner, int rowSize, Direction direction, double inRowGap, double inColGap) {
        super(corner, inRowGap, inColGap);
        this.rowSize = rowSize;
        computeDirections(direction);
    }

    public BoxEquiLayout(int rowSize, Direction direction, double inRowGap, double inColGap) {
    ...
    public void applyLayout(MathObjectGroup group) {
        ...
        for (MathObject obj : group) {
            hStep = Math.max(hStep, obj.getWidth());
            vStep = Math.max(vStep, obj.getHeight());
        }
        double h = hStep + inRowGap;
        double v = vStep + inColGap;
        switch (inRowStack) {
            case LEFT:
                hStep = -h;
                break;
...
//                group.get(n).stackTo(group.get(n - 1), inRowStack, this.inRowGap);
                group.get(n).stackTo(group.get(n - 1), Anchor.Type.CENTER);
                group.get(n).shift(hStep, 0);
            }

            if (rowCounter == this.rowSize) {
                rowCounter = 0;
                group.get(n).stackTo(firstOfTheRow, Anchor.Type.CENTER);
                group.get(n).shift(0, vStep);
    ...
    public BoxEquiLayout copy() {
        if (this.corner != null) {
            return new BoxEquiLayout(corner.copy(), rowSize, Direction.RIGHT_UP, inRowGap, inColGap);
        } else {
            return new BoxEquiLayout(rowSize, Direction.RIGHT_UP, inRowGap, inColGap);
        }
    }
